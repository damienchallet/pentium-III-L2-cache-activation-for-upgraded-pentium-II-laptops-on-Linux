/*  pIIIcache enables the L2 cache of PCs with buggy bioses, eg thinkpad 600
 */
#include <linux/module.h>  /* Needed by all modules */
#include <linux/kernel.h>  /* Needed for KERN_ALERT */
#include <linux/init.h>
#define DRIVER_AUTHOR "Damien Challet <damien.challet AT gmail.com>"
#define DRIVER_DESC   "Enables PIII L2 cache on PC whose BIOS does not support PIII. This hack uses very slightly modified code from LinuxBIOS.org project."
...
/* 		     signature); */
		printk("CPU signature of %x so no L2 cache configuration\n",
		     signature);
		goto done;
	}
	...
	if (signature == 0x630) {
		...
		if (eax == 0xc00000 || eax == 0x1000000) {
			printk( "Incorrect clock frequency ratio %x\n",
			       eax);
			goto bad;
		}
		...
		//printk("Sending %x to set_l2_register4\n", v);
...
//		printk("Maximum cache mask is %x\n", cache_setting);
...
//	printk("L2 Physical Address Range is %dM\n", (1 << r3) * 512);
